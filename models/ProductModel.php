...
class ProductModel {
    ...
    public function addProduct($data, $file) {
        ...
        $insert = $this->conn->query("
            INSERT INTO products(
                name, price, genre, author, year_published, language,
                number_pages, primary_description, secondary_description, image
            ) VALUES(
                '$name', '$price', '$genre', '$author', '$year', '$language',
                '$pages', '$primary', '$secondary', '$image'
            )
        ");
    ...
    public function deleteProduct($id) {
        ...
        $productRes = $this->conn->query("SELECT name, image FROM products WHERE id = '$id'");
        ...
        $deleted = $this->conn->query("DELETE FROM products WHERE id = '$id'");

        if ($deleted && isset($_SESSION['admin_id'])) {
            logAction(
                $this->conn,
                $_SESSION['admin_id'],
                'delete_product',
                'products',
                'Видалено товар: ' . ($product['name'] ?? ('ID ' . $id))
            );
        }
    }
